...
export async function POST(req, { params }) {
  ...
  // كل جداول الإدارة مفعّل عليها RLS، لذلك يجب أن يعمل هذا المسار بمفتاح الخدمة.
  if (!process.env.SUPABASE_URL || !process.env.SUPABASE_SERVICE_ROLE_KEY) {
    return NextResponse.json({
      error: 'اتصال قاعدة البيانات غير مضبوط على Vercel: أضف SUPABASE_URL و SUPABASE_SERVICE_ROLE_KEY ثم أعد النشر.'
    }, { status: 503 });
  }
