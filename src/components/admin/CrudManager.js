...
export default function CrudManager({
  ...
  async function save() {
    setBusy(true); setErr('');
    try {
      // لا نعدّل state مباشرة؛ هذا كان يسبب اختفاء بيانات النموذج/الخدمات قبل الإرسال.
      const formData = { ...modal.data };
      const serviceIds = Array.isArray(formData._serviceIds) ? [...formData._serviceIds] : undefined;
      delete formData._serviceIds;
      const payload = {
      ...
      try { data = text ? JSON.parse(text) : {}; } catch {
        throw new Error(`تعذر حفظ البيانات (HTTP ${res.status})`);
      }
      if (!res.ok) throw new Error(data.error || `تعذر الحفظ (HTTP ${res.status})`);
      setModal(null);
      router.refresh();
    } catch (e) {
      setErr(e.message);
  ...
  async function remove(row) {
    ...
    try {
      ...
      const text = await res.text();
      let data = {};
      try { data = text ? JSON.parse(text) : {}; } catch { throw new Error(`تعذر الحذف (HTTP ${res.status})`); }
      if (!res.ok) throw new Error(data.error || `تعذر الحذف (HTTP ${res.status})`);
      router.refresh();
    } catch (e) {
